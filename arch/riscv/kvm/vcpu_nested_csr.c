					    ...
					    unsigned long wr_mask)
{
	...
	unsigned long mode, host_mode, zero = 0, writeable_mask = 0;
	...
	case CSR_HGATP:
		...
		if (wr_mask & HGATP_MODE) {
			host_mode = kvm_riscv_gstage_mode(vcpu->kvm->arch.pgd_levels);
			...
			case HGATP_MODE_SV39X4:
				if (host_mode != HGATP_MODE_SV57X4 &&
				    host_mode != HGATP_MODE_SV48X4 &&
				    host_mode != HGATP_MODE_SV39X4)
					mode = HGATP_MODE_OFF;
				break;
#else
			case HGATP_MODE_SV32X4:
				if (host_mode != HGATP_MODE_SV32X4)
					mode = HGATP_MODE_OFF;
				break;
#endif
			default:
