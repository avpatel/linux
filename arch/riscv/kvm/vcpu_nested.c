					 ...
					 bool prev_priv)
{
	...
	nsc->htval = trap->htval;
	nsc->htinst = trap->htinst;
...
int kvm_riscv_vcpu_nested_init(struct kvm_vcpu *vcpu)
{
	...
	rc = kvm_riscv_vcpu_nested_swtlb_init(vcpu);
	if (rc)
		return rc;

	rc = kvm_riscv_vcpu_nested_csr_init(vcpu);
	if (rc) {
		kvm_riscv_vcpu_nested_csr_deinit(vcpu);
		return rc;
	}
