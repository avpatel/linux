...
void kvm_riscv_tlb_flush_process(struct kvm_vcpu *vcpu)
{
	...
	if (kvm_riscv_nacl_available())
		...
		kvm_riscv_local_hfence_gvma_vmid_all(vmid);
	kvm_riscv_vcpu_nested_gvma_flush_host(vcpu, 0, 0, 0);
}
...
{
	kvm_riscv_vcpu_nested_gvma_flush_guest(vcpu, 0, 0, 0);
}

void kvm_riscv_nested_hfence_vvma_all_process(struct kvm_vcpu *vcpu)
{
	kvm_riscv_vcpu_nested_vvma_flush_guest(vcpu, 0, 0, 0, -1UL);
}
		...
		case KVM_RISCV_HFENCE_GVMA_VMID_GPA:
			if (kvm_riscv_nacl_available())
				...
				kvm_riscv_local_hfence_gvma_vmid_gpa(d.vmid, d.addr,
								     d.size, d.order);
			kvm_riscv_vcpu_nested_gvma_flush_host(vcpu, d.addr, d.size, d.order);
			break;
